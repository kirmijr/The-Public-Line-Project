/*
 * A self-contained Fast Simplex Noise implementation.
 * Adapted for TypeScript and efficient 2D/3D noise generation.
 */
...
export class SimplexNoise {
  ...
  constructor(seed: number = Math.random()) {
    ...
    // Ensures that the same seed always produces the same permutation table
    let currentSeed = seed;
    
    // Normalize float seed (0-1) to integer range if necessary
    if (currentSeed < 1 && currentSeed > 0) {
        currentSeed = Math.floor(currentSeed * 4294967296);
    } else {
    ...
    // Shuffle using the deterministic PRNG
    for (let i = 255; i > 0; i--) {
  ...
  // 3D Simplex Noise (Useful for time-variant 2D noise)
